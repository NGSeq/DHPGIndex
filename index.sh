...
JAR_PATH="/media/dev/dev/panquery/target/scala-2.11/panquery_2.11-0.1.jar"
start=`date +%s`
...
#hdfs dfs -getmerge ${HDFS_PATH} ${INDEX_OUT}
...
start=`date +%s`
#./index/chic_index --threads=${THREADS} --kernel=${KERNEL} --verbose=5 --lz-input-file=${ORIG}/merged.lz ${INDEX_OUT}  ${MATCH_LEN} 2>&1 | tee ${OUTPUT_FOLDER}/index.log
