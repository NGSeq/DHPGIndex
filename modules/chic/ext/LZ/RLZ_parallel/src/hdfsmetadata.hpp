...
class HDFSMetaData {
 public:
  ...
  HDFSMetaData(string _fasta_filename, bool verbose=false) {
    cache_i = 0;
    ...
    int l = fasta_filename.length();
      char * fname = new char [l+1];
      strcpy(fname, fasta_filename.c_str());

    hfile = hdfsOpenFile(fs, fname, O_RDONLY |O_CREAT, 0, 0, 0);
      if(!hfile) {
          cout << "Cannot open: " << fasta_filename << endl;
      }
      cout << " opened HDFS file: " << fasta_filename << endl;
