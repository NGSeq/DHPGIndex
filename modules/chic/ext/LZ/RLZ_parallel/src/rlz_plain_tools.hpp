#ifndef RLZPLAINTOOLS_H
#define RLZPLAINTOOLS_H
#include <cassert>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
template <typename sa_t>
size_t process_block(uint8_t *buffer,
                     size_t buffer_len,
                     size_t block_offset,
                     size_t block_id,
                     FILE * fp,
...
#pragma omp parallel for
  for (size_t t = 0; t < n_partitions; t++) {
    ...
    chunk_print_info(block_id, t, abs_starting_pos, length, chunk_start_time, chunk_end_time, "");
  }
  long double end_time= wclock();
...
size_t parse_in_external_memory(char * input_filename,
  ...
  Dictionary<sa_t> d(reference_filename, reference_len, INT32_MAX);
  d.BuildSA();
 ...
 /* size_t text_len;
  ...
  while (block_offset < text_len) {
    ...
    n_factors += process_block(buffer,
                               buffer_len,
                               block_offset,
                               block_id,
                               fp,
