...
int main(int argc, char **argv) {
    ...
    if(opt==0) {
        ...
        size_t sparse_sample_limits_kernel_len = (nphrases+1)/sparse_sample_ratio;
        if ((nphrases+1)%sparse_sample_ratio)
            sparse_sample_limits_kernel_len++;
        vector<uint64_t> sparse_sample_limits_kernel = vector<uint64_t>(sparse_sample_limits_kernel_len);
        ...
        store_to_file(sparse_sample_limits_kernel, "merged.sparse_limits_kernel");
    }
