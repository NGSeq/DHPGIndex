...
object DistributedRLZDFS {
  ...
  def main(args: Array[String]) {
   ...
   //println("writing local")
    ...
    def binarySearch(lb: Int, rb: Int, d_b: Broadcast[String], cur: Char, i: Int, SA_b: ArrayBuffer[Int]): Option[(Int, Int)] = {
      val d = d_b.value
      var low = lb
      var high = rb
      while (low < high) {
      ...
      if ((SA_b(low_res) + i)>= d.length || d(SA_b(low_res) + i) != cur) {
        return None
      }
      high = rb
      while (low < high) {
      ...
      if (SA_b(low) != d.length() - 1 && SA_b(low)+i< d.length() && d(SA_b(low) + i) != cur) {
        //if(low_res>low-1) {
    ...
    // to newline
    def factor(i: Int, x: String, d_b: Broadcast[String], SA_b: ArrayBuffer[Int]): (String, Long) = {
      val d = d_b.value
    ...
    def encode(x: String, d_b: Broadcast[String]): ArrayBuffer[(String, Long)] = {
      ...
      val fss = FileSystem.get(new Configuration())
      val fst = fss.listStatus(new Path("/user/root/radix/"))
      var fileIndex=0
      var lastpos = fst(fileIndex).getPath.getName.split("_")(1).toInt

      var hdfsstream = client.open("/user/root/radix/radix_"+lastpos)
      val bfr = new BufferedReader(new InputStreamReader(hdfsstream))

      val SA_b = new ArrayBuffer[Int]()

      var line: String = null
      while ({line = bfr.readLine; line != null}) {
        SA_b.append(line.toInt)
      }
      ...
      while (i < x.length()) {
        ...
        if (i > lastpos){
          fileIndex += 1
          lastpos = fst(fileIndex).getPath.getName.split("_")(1).toInt
          hdfsstream = client.open("/user/root/radix/radix_"+lastpos)
    ...
    val encoded = nonParsedRef.map{x =>
      ...
      val encodings = encode(x._4,parsedRef)
      //if(x._2==0) {
