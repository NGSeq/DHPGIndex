...
object DistributedRLZSplitArray {
  ...
  def main(args: Array[String]) {
   ...
   //println("writing local")
    ...
    def binarySearch(lb: Long, rb: Long, ref: Array[String], cur: Char, i: Int, SA_b: Array[Array[Long]]): Option[(Long, Long)] = {
      ...
      while (low < high) {
        ...
        if (mid > rlen){
          newmid = mid-rlen
          midKey = SA_b(1)(newmid.toInt) + i
        }
        else
          midKey = SA_b(0)(mid.toInt) + i
      ...
      val sabsum = SA_b(saind)(newlow_res.toInt)+i
      ...
      val sbs = SA_b(sind)(newlow.toInt)+i
      var refind = 0
      var bind = sbs
      if (bind > rlen){
        refind = 1
      ...
      if (SA_b(sind)(newlow.toInt) != (reflength - 1) && sbs < reflength && rbase != cur) {
        //if(low_res>low-1) {
    ...
    // to newline
    def factor(i: Int, split: String, ref: Array[String], SA_b: Array[Array[Long]], reflen: Long): (String, Int) = {
    ...
    def encode(split: String, reflen: Long): ArrayBuffer[(String, Int)] = {
      ...
      val SA_b = Array(SA_b1.toArray,SA_b2.toArray)
      val ref = Array(ref1,ref2)
