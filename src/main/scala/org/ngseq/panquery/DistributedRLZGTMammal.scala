...
object DistributedRLZGTMammal {
  ...
  def main(args: Array[String]) {
    ...
    val data = spark.sparkContext.textFile(dataPath).zipWithIndex().filter(x=>x._1.length!=0)

    val splitted = data.map{rec=>

      if(rec._1.indexOf(System.lineSeparator)>0) {
        val id = rec._2
        ...
        if (taxsplit.length > 0){
          ...
          if(taxsplit(0).split(".").length>1)
            groupname=taxsplit(0).split(".")(0).replaceAll("[^A-Za-z0-9]", "")
          if(taxsplit(0).split("_").length>1)
            groupname=taxsplit(0).split("_")(0).replaceAll("[^A-Za-z0-9]", "")
        }
        if(groupname.length>30)
          groupname = groupname.substring(0,29)
        ...
        groupname += "_" + chr
        if(!seqname.toLowerCase.contains("complete genome")){
           (0,0,"","")
        }
        (id,seq.length,seq, groupname)
      }else{
        System.err.println("Invalid data! " +rec)
        (0,0,"","")
      }

    }.filter(r=>r._4.nonEmpty).groupBy(g=>g._4).persist(StorageLevel.MEMORY_ONLY_SER)
    ...
    splitted.foreach{group=>
      ...
      var dictrefs = (group._2.size/refsize)+1
      if(dictrefs>maxrefs)
        dictrefs = maxrefs
      val refs = group._2.take(dictrefs)
      val reffile = "/mnt/tmp/ref"+group._1
      val pw = new PrintWriter(new File(reffile))
      refs.foreach(x => pw.write(x._2))
      pw.close()
