...
object DistributedRLZGroupTax2 {
  ...
  def main(args: Array[String]) {
    ...
    val splitted = data.foreachPartition(part=> {

      val grouped = part.map(rec=>{

        val seqname = rec.substring(1,rec.indexOf(System.lineSeparator))
        val seq = rec.substring(rec.indexOf(System.lineSeparator)).trim
        ...
        Tuple3(seq.length,seq, groupname)

    }).toTraversable.groupBy(k=>k._3)
    ...
    grouped.foreach { group =>
      ...
      println("Creating Suffix Array frogrem reference sequence of length" + reflength)
     ...
     group._2.foreach(sample=>{
        // encodings.appendAll(encode(sample._2))
