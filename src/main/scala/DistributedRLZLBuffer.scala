...
object DistributedRLZLBuffer {
  ...
  def main(args: Array[String]) {
   ...
   //println("writing local")
    ...
    def binarySearch(lb: Long, rb: Long, ref: LArray[Byte], cur: Char, i: Int, SA_b: LArray[Byte]): Option[(Long, Long)] = {
      ...
      if ((SA_b(low_res).toLong  + i)>= ref.length || ref(SA_b(low_res).toLong + i) != cur) {
        return None
      }
      high = rb
      while (low < high) {
      ...
      if (SA_b(low).toLong != ref.length - 1 && SA_b(low).toLong+i< ref.length && ref(SA_b(low).toLong + i).toChar != cur) {
        //if(low_res>low-1) {
    ...
    // to newline
    def factor(i: Int, split: String, ref: LArray[Byte], SA_b: LArray[Byte]): (String, Int) = {
      ...
      breakable {
        while (j < split.length()) {
          ...
          if (lb == rb && ref(SA_b(lb).toLong + j - i) != split(j)) {
            break
          }
    ...
    def encode(split: String, reflen: Long): ArrayBuffer[(String, Int)] = {
      ...
      val client = new DFSClient(URI.create("hdfs://ambari2.novalocal:8020"), new Configuration())
      ...
      val SA_b =  LArray.of[Byte](reflen)

      var line: String = bfr.readLine()

      var l = 0L
      while (line != null) {
        val bytes = line.stripLineEnd.getBytes()
        bytes.foreach(b => {
          SA_b(l)=b
          l+=1
        })
      ...
      val ref =  LArray.of[Byte](reflen)
      var value: Int = bfref.read()
      var cnt = 0
      while ( {value != -1}) {

        ref(cnt)=value.toByte
        cnt+=1
        value = bfref.read()
      }
