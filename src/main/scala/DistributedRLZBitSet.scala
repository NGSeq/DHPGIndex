...
object DistributedRLZLarray {
  ...
  def main(args: Array[String]) {
   ...
   //println("writing local")
    ...
    def binarySearch(lb: Long, rb: Long, ref: LArray[Char], cur: Char, i: Int, SA_b: LBuffer): Option[(Long, Long)] = {
      ...
      if ((SA_b.getLong(low_res) + i)>= ref.length || ref(SA_b.getLong(low_res) + i) != cur) {
        return None
      }
      high = rb
      while (low < high) {
      ...
      if (SA_b.getLong(low) != ref.length - 1 && SA_b.getLong(low)+i< ref.length && ref.getLong(SA_b(low) + i) != cur) {
        //if(low_res>low-1) {
    ...
    // to newline
    def factor(i: Int, split: String, ref: LArray[Char], SA_b: LBuffer): (String, Int) = {
      ...
      breakable {
        while (j < split.length()) {
          ...
          if (lb == rb && ref(SA_b.getLong(lb) + j - i) != split(j)) {
            break
          }
      ...
      if (j == i) {
        ...
        //println(i+","+j)
        return (SA_b.getLong(lb).toString(), j - i)
      }
    }
    ...
    def encode(split: String, reflen: Long): ArrayBuffer[(String, Int)] = {
      ...
      val SA_b =  new xerial.larray.buffer.LBuffer(reflen)

      var line: String = bfr.readLine()

      var l = 0L
      while (line != null) {
        SA_b.putLong(l,java.lang.Long.valueOf(line.stripLineEnd))
        line = bfr.readLine()
        l+=1
      }
