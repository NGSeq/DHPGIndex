...
object DistributedRLZLarrayBigInt {
  ...
  def main(args: Array[String]) {
   ...
   //println("writing local")
    ...
    def binarySearch(lb: Long, rb: Long, ref: LArray[Char], cur: Char, i: Int, SA_b: LArray[BigInteger]): Option[(Long, Long)] = {
      ...
      while (low < high) {
        ...
        val midKey = SA_b(mid).add(BigInteger.valueOf(i.toLong)).asInstanceOf[Long]
      ...
      if ((SA_b(low_res).add(BigInteger.valueOf(i.toLong)).asInstanceOf[Long])>= ref.length || ref(SA_b(low_res).add(BigInteger.valueOf(i.toLong)).asInstanceOf[Long]) != cur) {
        return None
      }
      high = rb
      while (low < high) {
      ...
      if (SA_b(low).asInstanceOf[Long] != ref.length - 1 && SA_b(low).add(BigInteger.valueOf(i.toLong)).asInstanceOf[Long]< ref.length && ref(SA_b(low).add(BigInteger.valueOf(i.toLong)).asInstanceOf[Long]) != cur) {
        //if(low_res>low-1) {
    ...
    // to newline
    def factor(i: Int, split: String, ref: LArray[Char], SA_b: LArray[BigInteger]): (String, Int) = {
      ...
      breakable {
        while (j < split.length()) {
          ...
          if (lb == rb && ref(SA_b(lb).add(BigInteger.valueOf(j.toLong)).subtract(BigInteger.valueOf(i.toLong)).asInstanceOf[Long]) != split(j)) {
            break
          }
    ...
    def encode(split: String, reflen: Long): ArrayBuffer[(String, Int)] = {
      ...
      val SA_b =  LArray.of[BigInteger](reflen)

      var line: String = bfr.readLine()

      var l = 0L
      while (line != null) {
        SA_b(l)=BigInteger.valueOf(java.lang.Long.valueOf(line.stripLineEnd))
        line = bfr.readLine()
        l+=1
      }
