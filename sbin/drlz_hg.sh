...
spark-submit --master $MASTER --conf spark.scheduler.minRegisteredResourcesRatio=0.3 --conf spark.executor.cores=$CORES --conf spark.dynamicAllocation.cachedExecutorIdleTimeout=240 --conf spark.dynamicAllocation.executorAllocationRatio=0.5 --conf spark.scheduler.mode=FIFO --conf spark.driver.memory=${DRMEM} --conf spark.executor.memory=${EXMEM} --conf spark.dynamicAllocation.enabled=true --conf spark.shuffle.service.enabled=true --conf spark.port.maxRetries=200 --conf spark.executor.memoryOverhead=1g --conf spark.driver.maxResultSize=6000m --conf spark.executor.heartbeatInterval=100 --conf spark.network.timeout=86400 --class org.ngseq.dhpgidx.DistributedRLZFasta ../target/dhpgidx-0.1-jar-with-dependencies.jar $1 ${HDFSPATH}/*.$i $HDFSURL $OUT $REFS $PARTITIONS &> DRLZFasta.$1.log
